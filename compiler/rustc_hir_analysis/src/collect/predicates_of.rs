use crate::astconv::{AstConv, AstConvBase, AstConvBaseExt};
...
fn gather_explicit_predicates_of(tcx: TyCtxt<'_>, def_id: DefId) -> ty::GenericPredicates<'_> {
    ...
    for predicate in ast_generics.predicates {
        match predicate {
            hir::WherePredicate::BoundPredicate(bound_pred) => {
                ...
                let mut bounds = Bounds::default();
                icx.add_bounds(ty, bound_pred.bounds.iter(), &mut bounds, bound_vars);
                predicates.extend(bounds.predicates());
            }

            hir::WherePredicate::RegionPredicate(region_pred) => {
                let r1 = icx.ast_region_to_region(&region_pred.lifetime, None);
                predicates.extend(region_pred.bounds.iter().map(|bound| {
                    let (r2, span) = match bound {
                        hir::GenericBound::Outlives(lt) => {
                            (icx.ast_region_to_region(lt, None), lt.ident.span)
                        }
                        _ => bug!(),
                    };
    ...
    if let Node::Item(&Item { kind: ItemKind::OpaqueTy(..), .. }) = node {
        ...
        for (arg, duplicate) in std::iter::zip(lifetimes, ast_generics.params) {
            ...
            let orig_region = icx.ast_region_to_region(&arg, None);
...
pub(super) fn super_predicates_that_define_assoc_type(
    ...
    if trait_def_id.is_local() {
        ...
        let superbounds1 = if let Some(assoc_name) = assoc_name {
            icx.compute_bounds_that_match_assoc_type(self_param_ty, bounds, assoc_name)
        } else {
            icx.compute_bounds(self_param_ty, bounds)
        };
